...
gsort -alpha1
local top5ind   " `=ind[1]' `=ind[2]' `=ind[3]' `=ind[4]' `=ind[5]'  `=ind[6]' `=ind[7]' `=ind[8]' `=ind[9]' `=ind[10]' "
...
file open fh  using "../results/rotemberg_summary_bar`file_suffix'.tex", write replace
...
graph export "../results/overid_BAR`file_suffix'.pdf", replace
...
graph export "../results/F_vs_rotemberg_weight_BAR`file_suffix'.pdf", replace
...
file write fh "\multicolumn{3}{l}{\textbf{Panel A: Top 10 Rotemberg weight industries (baseline)} }\\" _n
file write fh  " & $\hat{\alpha}_{k}$   & Ind Share \\ \cmidrule(lr){2-3}" _n
foreach ind in `top5ind' {
	file write fh  "`ind_name_`ind'' & `alpha_`ind''  & `share_`ind'' \\ " _n
	}
file close fh
...
file open fh  using "../results/rotemberg_summary_bar`file_suffix'.tex", write append
file write fh "\multicolumn{3}{l}{\textbf{Panel B: Top 10 Rotemberg weight industries (no demeaning)} }\\" _n
file write fh  " & $\hat{\alpha}_{k}$ & Ind Share \\ \cmidrule(lr){2-3}" _n
foreach ind in `top5ind' {
	file write fh  "`ind_name_`ind'' & `alpha_`ind''  & `share_`ind'' \\ " _n
	}
...
/** Demean growth rates **/
egen mean_growth = rowmean(`growth_stub'*)
egen mean_growth2 = mean(mean_growth), by(czone)
foreach growth of varlist `growth_stub'* {
	qui replace `growth' = `growth'  - mean_growth2
	}
drop mean_growth
...
/*** Write final table **/
file open fh  using "../results/rotemberg_summary_bar`file_suffix'.tex", write append
file write fh "\multicolumn{3}{l}{\textbf{Panel C: Top 10 Rotemberg weight industries (simple demeaning)} }\\" _n
file write fh  " & $\hat{\alpha}_{k}$  & Ind Share \\ \cmidrule(lr){2-3}" _n
foreach ind in `top5ind' {
	file write fh  "`ind_name_`ind'' & `alpha_`ind''  & `share_`ind'' \\ " _n
	}
