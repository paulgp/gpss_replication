...
foreach var of varlist `ind_stub'1 `ind_stub'5 `ind_stub'2 `ind_stub'6 `ind_stub'31 {
	if regexm("`var'", "`ind_stub'(.*)") {
		local ind = regexs(1) 
		}
	tempvar temp
	qui gen `temp' = `var' * `growth_stub'`ind'
	ch_weak, p(.05) beta_range(-0.5(.01)0.5)   y(`y') x(`x') z(`temp') weight(`weight') controls(`controls') cluster(rmsa)
...
file open fh  using "../results/rotemberg_summary_card_hs.tex", write replace
...
graph export "../results/overid_CARD_hs.pdf", replace
...
graph export "../results/F_vs_rotemberg_weight_CARD_hs.pdf", replace
